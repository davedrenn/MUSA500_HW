...
finallogitoutput <- cbind(logitoutput$coefficients,or_ci)
finallogitoutput
...
```
The table below shows the sensitivity, specificty, and miss classification rate for different tresholds. Overall, as the threshold increases our sensitivity and miss-classification rate decrease. As the threshold increases, the specifity also increases. The threshold with the lowest midd classification rate is 0.5 threshold. The threshold with the highest miss classification rate is 0.02. If we consider just the miss classification rate, the 0.50 threshold provides the best results.
...
cbind(tresholds,sens,spec,mis_class) %>%
  as_data_frame() %>%
  kbl(col.names=c("Threshold","Sensitivity (%)","Specificty (%)","Miss Classification Rate (%)")) %>%
  kable_classic_2() %>%
...
abline(a=0,b=1)
```
We now use the minimum distance approach to determine the threshold where the distance from the top left hand corner to the ROC curve is lowest.
