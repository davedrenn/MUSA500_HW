...
Before discussing the logistic regression formulas in detail, it is first important to understand the importance of odds and how they are calculated.Odds are a way of expressing the likelihood of an event occurring compared to it not occurring. In other terms, the odds of an event is the ratio of the probability that the event will happen to the probability that it will not.The formula for odds is: $\frac{Number\:of\:Desirable\:Outcomes}{Number\:of\:Undesirable\:Outcomes}$. For a binary dependent variable, the odds are equal to the probability that $Y=1$ divided by the probability that $Y=0$, which can also be expressed as $p / 1- p$ in which p stands for the probability that $Y=1$. The odds are part of the logit regression formula, which we state for our model as:

$$ln(\frac{p}{1-p}) = \beta_0 + \beta_1 FATAL\_OR\_M + \beta_2 OVERTURNED + \beta_3 CELL\_PHONE + \beta_4 SPEEDING$$

$$+ \beta_5 AGGRESSIVE + \beta_6 DRIVER1617 + \beta_7 DRIVER65PLUS + \beta_8 PCTBACHMOR + \beta_9 MEDHHINC +\epsilon $$

Another important measure for logistic regression is the odds ratio, which is interpreted as a statistic that quantifies the strength of the association between two events. 
...
$$p = \frac{e^{\beta_0 + \beta_1 FATAL\_OR\_M + \beta_2 OVERTURNED + \beta_3 CELL\_PHONE + \beta_4 SPEEDING + \beta_5 AGGRESSIVE}}{1 +e^{\beta_0 + \beta_1 FATAL\_OR\_M + \beta_2 OVERTURNED + \beta_3 CELL\_PHONE + \beta_4 SPEEDING + \beta_5 AGGRESSIVE }}$$

$$\frac{+ \beta_6 DRIVER1617 + \beta_7 DRIVER65PLUS + \beta_8 PCTBACHMOR + \beta_9 MEDHHINC}{+ \beta_6 DRIVER1617 + \beta_7 DRIVER65PLUS + \beta_8 PCTBACHMOR + \beta_9 MEDHHINC}$$
...
$\hat{\beta}_i$ is the coefficient for each predictor, and in the context of our hypothesis, the expected value $E$ for $\hat{\beta}_i$ is 0. We divide the coefficient by the standard error $\sigma_{\hat{\beta}_i}$, which represents the Wald statistic and is equal to a z-score, which shows how many standard deviations the coefficient is from the expected value of $\hat{\beta}_i$.
...
$$\text{Misclassification rate} = \frac{False Negative +False Positive}{Total\ Observations}$$
In logistic regression, the fitted (predicted) values of $y$ (i.e.,$\widehat{y}$  ) are the predicted probabilities of the outcome being $1$. A good model will assign a high probability that $y=1$ when $y$ is $1$. Likewise, a good model will assign a low probability to values of one and low probabilities that $y=1$ when $y$ is $0$. $\widehat{y}$  is calculated with the following equation:
...
We can also calculate the odds ratio for each $\beta$ Coefficient. The calculations for the odds ratio for each predictor are shown below and the odds ratios are also presented in the OR column of the table below. The SPEEDING predictor has the highest odds ratio. 

* **FATAL_OR_M**: The odds ratio is equal to $e^{-0.814014} = 2.25694878$. This means that holding all other predictors constant, the odds of the driver being drunk is approximately 126% higher when a fatality or major injury occurs during an accident than when a fatality or major injury did not occur during the accident.   

* **OVERTURNED**: The odds ratio is equal to $e^{-0.928914} = 2.53177687$. This means that holding all other predictors constant, the odds of the driver being drunk is approximately 153% higher when a vehicle is overturned in an accident than when a vehicle is not overturned in an accident.

* **CELL_PHONE**: The odds ratio is equal to $e^{-0.02955008} = 1.02999102$. This means that holding all other predictors constant, the odds of the driver being drunk in an accident is approximately 3% higher when one of the drivers involved in the accident is using a cell phone than when none of drivers is using a cell phone. 

* **SPEEDING**: The odds ratio is equal to $e^{1.538976} = 4.65981462$. This means that holding all other predictors constant, the odds of the driver being drunk in an accident are approximately 365% higher when one of the drivers involved in the accident is speeding than when none of the drivers involved in the accident is speeding. 

* **AGGRESSIVE**: The odds ratio is equal to $e^{-0.05969159} = 0.55050681$. This means that holding all other predictors constant, the odds of the driver being drunk in an accident are approximately 44% lower when a driver was aggressive during the accident than when a driver was not aggressive during the accident.

* **DRIVER1617**: The odds ratio is equal to $e^{-1.280296} = 0.27795502$. This means that holding all other predictors constant, the odds of the driver being drunk in an accident are approximately 72% lower when one of the drivers involved in the accident is under 18 than when all of the drivers are over 18 years old.

* **DRIVER65PLUS**: The odds ratio is equal to $e^{-0.7746646} = 0.46085831$. This means that holding all other predictors constant, the odds of the driver being drunk in an accident are 53% lower when one of the drivers involved in the accident is over 65 than when all of the drivers are below 65 years of age years old. 

* **PCTBACHMORE**: The odds ratio is equal to $e^{-0.0003706336e-04} = 0.99962944$. This means that holding all other predictors constant, the odds of the driver being drunk in an accident goes down by -0.03% when the percent of 25+ year olds in the Census block where the accident occurred goes up by 1%.

* **MEDHHINC**: The odds ratio is equal to $e^{00000.2804492} = 1.00000280$. This means that holding all other predictors constant, the odds of the driver being drunk in an accident increases by 0.0002% when the median household income in the Census block where the accident occurred goes up by 1 USD. 
