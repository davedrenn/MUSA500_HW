---
title: "Assignment 2 - Spatial Regression"
...
$$LNMEDHVAL = \rho WLNMEDHVAL + \beta_0 + \beta1 PCTVACANT + \beta2 PCTSINGLES$$ 
$$ +\ \beta3 PCTBACHMOR + \beta4 LNNBELPOV + \epsilon$$
Spatial lag regression includes the spatial lag term of the dependent variable $y$ (e.g. LNMEDHVAL) as a predictor in addition to the terms included in an OLS regression. The coefficient rho, denoted by $p$, is limited to a value between $-1$ and $1$. The predictor $Wy$ represents the weights matrix $W$ applied to the dependent variable $y$, which allows us to associate the nearby $ys$ (the log of median home values in nearby Census block groups) with an individual observation. Taken together, rho and $Wy$ are the spatial lag of $y$ as a predictor for $y$.
...
$$LNMEDHVAL = \beta_0 + \beta1 PCTVACANT + \beta2 PCTSINGLES$$
$$+\ \beta3 PCTBACHMOR + \beta4 LNNBELPOV + \lambda W\epsilon + u$$
...
ggplot()+
  ...
  annotate("text",x=0.4, y=150,color='red',label=paste("Moran's I = ",as.character(round(moranMC$statistic,2))))+
...
ggplot()+
  geom_sf(data=data,aes(fill=LNMEDHVAL),linewidth=0.2,color='grey60')+
...
ggplot()+
  geom_sf(data=data_LISA,aes(fill = cut(Pr.z....E.Ii.., breaks = c(0,0.01,0.05,0.5,1))),linewidth=0.2,color='grey60')+
...
ggplot()+
  geom_sf(data=data_LISA,aes(fill=quadrant),linewidth=0.2,color='grey60')+
