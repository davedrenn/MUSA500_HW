...
gwrmodel<-as.data.frame(gwrmodel$SDF)
...
### Standardized Coefficients Map

```{r warning=FALSE, message=FALSE, cache=FALSE}

data$coefLNNBELPOVst<-gwrmodel$LNNBELPOV/gwrmodel$LNNBELPOV_se
data$coefPCTBACHMORst<-gwrmodel$PCTBACHMOR/gwrmodel$PCTBACHMOR_se
data$coefPCTSINGLESst<-gwrmodel$PCTSINGLES/gwrmodel$PCTSINGLES_se
data$coefPCTVACANTst<-gwrmodel$PCTVACANT/gwrmodel$PCTVACANT_se
...
coefPCTBACHMOR<-tm_shape(data)+
  tm_fill(col='coefPCTBACHMORst', breaks=c(-Inf, -6, -4, -2, 0, 2, 4, 6, Inf), title='Standardized coefficient of PCTBACHMOR', 
          palette='-RdBu')+
  tm_layout(frame=FALSE, title = 'Percentage of Bachelors or Higher')

coefPCTSINGLES<-tm_shape(data)+
  tm_fill(col='PCTSINGLES', breaks=c(-Inf, -6, -4, -2, 0, 2, 4, 6, Inf), title='Standardized coefficient of PCTSINGLES', 
          palette='-RdBu')+
