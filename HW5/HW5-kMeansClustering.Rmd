...
We use the K-means clustering method to uncover patterns and relationships within the data. K-means clustering can help identify distinct groups of block groups with similar characteristics, facilitating a deeper understanding of the spatial and socioeconomic landscape of Philadelphia. Through this analysis, we can answer questions such as:

- Are there distinct neighborhoods in terms of socioeconomic status?
- How does educational attainment correlate with housing values and income levels?
- Are there areas with notably high vacancy rates, and how do they relate to other factors?  
This clustering approach can reveal insights into the city's urban structure.
...
- Randomly select K data points as cluster
...
NbClust identifies 2 as the optimal numbers of clusters based on criteria from 26 of the methods considered most relevant by the package. By plotting the outcomes, we see that 8 of the 26 methods chose 2 as the optimal number of clusters. The next highest chosen number is 15 clusters, which was chosen by 6 of the 26 methods.
...
By mapping our observations based on the two clusters, we observed that our K-means clusters appear to also cluster in space, indicating likely spatial autocorrelation. Generally speaking, there are clusters around Center City, Northwest Philly, and Northeast Philly, with smaller clusters in University City, the northern edge of North Philly and the western Edge of West Philly.
